...
def fetch_ticker_data(symbol):
    try:
        # Use auto_adjust and explicit period for stable index fetching
        hist = yf.Ticker(symbol).history(period="10y", auto_adjust=True)
        
        # Fallback if history() returns empty dataframe
        if hist.empty:
        ...
        # Safe timezone stripping to avoid pandas IndexErrors
        if hasattr(hist.index, "tz") and hist.index.tz is not None:
            hist.index = hist.index.tz_convert(None)
            
        # Flatten MultiIndex columns if generated by yf.download
        if isinstance(hist.columns, pd.MultiIndex):
            return hist['Close'][symbol] if symbol in hist['Close'] else hist['Close'].iloc[:, 0]

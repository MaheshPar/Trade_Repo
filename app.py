...
# Global variable to store current scan data
results = []
...
if st.button("SCAN"):
    ...
    with st.spinner("Scanning markets..."):
        ...
        # Save results directly to session state to prevent them from vanishing on tab switches
...
# Retrieve scanned data if exists
...
with tab3:
    ...
    st.markdown("""
    **Strategy Rules:**
    * **Target Universe**: `MID150BEES.NS`, `GOLDBEES.NS`, `CPSEETF.NS`, `HNGSNGBEES.NS`, `ITBEES.NS`, `NIFTYBEES.NS`, `NV20IETF.NS`, `BANKBEES.NS`, `PHARMABEES.NS`, `JUNIORBEES.NS`
    * **Rebalancing**: Dynamic 1-Month intervals (first trading day of each month).
    * **Allocation Strategy**: Invests in the **Top 5 symbols** holding the highest previous 1-Month returns (20% weight each).
    """)
    ...
    if st.button("RUN 5-YEAR BACKTEST"):
        with st.spinner("Processing historical pricing databases..."):
            # Fetch 5 years + 2 months buffer to calculate initial momentum return
            ...
            if df_prices.empty or len(df_prices.columns) < 2:
                ...
                bt_history.append({
                    "Date": first_inv_date,
                    "Top-5 Momentum": portfolio_val,
                    "Nifty 50 Benchmark": nifty_val,
                    "Equal-Weight Portfolio": eq_weight_val,
                    "Allocated Holdings": "Initial State"
                })

                # Compute monthly rebalances
                for t in range(1, len(monthly_dates) - 1):
                    d_prev = monthly_dates[t-1]
                    d_curr = monthly_dates[t]
                    d_next = monthly_dates[t+1]

                    # 1. Calculate historical 1-month returns
                    m_returns = {}
                    ...
                    # 2. Pick top 5 based on returns
                    sorted_m = sorted(m_returns.items(), key=lambda x: x[1], reverse=True)
                    top_5 = [item[0] for item in sorted_m[:5] if item[1] != -999.0]

                    if not top_5:
                        top_5 = backtest_symbols[:5]

                    # 3. Calculate portfolio step returns
                    # A. Momentum Return
                    p_ret_sum = 0.0
                    v_count = 0
                    for sym in top_5:
                    ...
                    # Update cumulative compounding values
                    ...
                    bt_history.append({
                        ...
                        "Nifty 50 Benchmark": nifty_val,
                        "Equal-Weight Portfolio": eq_weight_val,
                        "Allocated Holdings": ", ".join([s.replace(".NS", "") for s in top_5])
                    })
                ...
                cagr_strat = (df_bt["Top-5 Momentum"].iloc[-1] / 100.0) ** (1 / years_dur) - 1 if years_dur > 0 else 0
                ...
                with m_col1:
                    st.metric("Top-5 Momentum CAGR", f"{cagr_strat*100:.2f}%", f"vs Nifty: {(cagr_strat-cagr_nifty)*100:+.2f}%")
                with m_col2:
                ...
                chart_df = df_bt.set_index("Date")[["Top-5 Momentum", "Nifty 50 Benchmark", "Equal-Weight Portfolio"]]
                st.line_chart(chart_df, height=450)

                # Rebalance Decisions Log
                st.markdown("### 🗓️ Monthly Rebalance Log")
                log_df = df_bt.copy()
                log_df["Date"] = log_df["Date"].dt.strftime('%B %Y')
                log_df = log_df[["Date", "Allocated Holdings", "Top-5 Momentum", "Equal-Weight Portfolio", "Nifty 50 Benchmark"]]

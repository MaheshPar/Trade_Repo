# added buffer 2 stock rank in backtesting top 3 + 2
...
# Recovery and declaration of system states
...
if st.button("SCAN"):
    ...
    with st.spinner("Scanning markets..."):
        ...
        results = []
        ticker_names = {t["symbol"]: t["name"] for t in st.session_state.etf_tickers}
        for idx, item in enumerate(st.session_state.etf_tickers):
            ...
            past_1m_date = pd.to_datetime(to_date) - pd.DateOffset(months=1)
            past_1m_price = get_price_on_or_before(series, past_1m_date)
            raw_1m = ((lcp - past_1m_price) / past_1m_price) if (lcp is not None and past_1m_price is not None and past_1m_price > 0) else -999.0

            row = {
                "Sr. No.": idx + 1,
                "Ticker": sym,
                "Name": ticker_names.get(sym, ""),
...
# DYNAMIC TREND LEADERS DISPLAY
if scanned_data:
    trend_candidates = [r for r in scanned_data if r["Ticker"] != "^NSEI" and r["_raw_1m_return"] != -999.0]
    
    if trend_candidates:
        st.markdown("### 🔥 Market Momentum Leaders (1-Month Return)")
        trend_toggle = st.radio("Display Leaders:", ["Top 3", "Top 5"], horizontal=True, key="trend_toggle")
        num_leaders = 3 if trend_toggle == "Top 3" else 5
        
        sorted_leaders = sorted(trend_candidates, key=lambda x: x["_raw_1m_return"], reverse=True)[:num_leaders]
        
        cols = st.columns(num_leaders)
...
with tab1:
    if scanned_data:
        ...
        columns_order = [
            "Sr. No.", "Ticker", "Name", "LCP", "50 EMA", "100 EMA", "200 EMA", 
...
with tab3:
    ...
    with ctrl_col1:
        rebal_choice = st.selectbox(
            ...
            index=1  # Default to 3 Months as requested
        )
    with ctrl_col2:
        top_choice = st.selectbox(
            "Allocation Selection Size", 
            options=["Top 3", "Top 5"], 
            index=0  # Default to Top 3
        )
    ...
    # Establish dynamic buffer value based on selection criteria
    buffer_n_val = 5 if top_n_val == 3 else 7

    st.markdown(f"""
    **Current Backtest Rules:**
    * **Target Universe**: 10 Core ETFs.
    * **Holding Period**: Rebalancing executed once every **{rebal_choice}**.
    * **Ranking Rule**: Initial allocation targets the **{top_choice}** highest-performing ETFs based on trailing 1-month returns.
    * **Retention Buffer**: An existing position is **only sold** if its momentum rank drops outside the **Top {buffer_n_val}**.
    """)
    ...
    if st.button("RUN BACKTEST"):
        with st.spinner("Processing historical pricing databases..."):
            ...
            if df_prices.empty or len(df_prices.columns) < 2:
                ...
                bt_history = []
                current_holdings = []  # Tracks assets persistently across rebalances
                
                t = 1
                d_next = monthly_dates[t]
                ...
                while t < len(monthly_dates) - rebal_step:
                    ...
                    # 1. Calculate trailing 1-month returns for momentum ranking at this date
                    m_returns = {}
                    ...
                    # Generate absolute momentum rankings across all active assets
                    sorted_m = [item[0] for item in sorted(m_returns.items(), key=lambda x: x[1], reverse=True) if item[1] != -999.0]
                    
                    # Apply Retention Buffer Filter: Keep existing holdings if they remain within the Top 5 (or Top 7)
                    retained_holdings = [sym for sym in current_holdings if sym in sorted_m and sorted_m.index(sym) < buffer_n_val]
                    
                    # Fill vacancies with highest ranked available assets
                    slots_needed = top_n_val - len(retained_holdings)
                    fill_list = []
                    for sym in sorted_m:
                        if len(fill_list) >= slots_needed:
                            break
                        if sym not in retained_holdings:
                            fill_list.append(sym)
                            
                    top_selected = retained_holdings + fill_list

                    if not top_selected:
                        top_selected = backtest_symbols[:top_n_val]
                        
                    # Commit selection state to historical memory module
                    current_holdings = top_selected.copy()

                    # Compute trailing Nifty 50 1M Return at d_curr
                    n_prev_1m = df_prices.loc[d_prev, "^NSEI"] if "^NSEI" in df_prices.columns else None
                    n_curr_1m = df_prices.loc[d_curr, "^NSEI"] if "^NSEI" in df_prices.columns else None
                    nifty_1m_pct = ((n_curr_1m - n_prev_1m) / n_prev_1m) * 100 if (n_prev_1m and n_curr_1m and n_prev_1m > 0) else 0.0

                    # 2. Build row entry capturing state AT the rebalance decision moment
                    row_data = {
                        "Date": d_curr,
                    ...
                    # Calculate returns dynamically for active positions in allocated order
                    for i in range(top_n_val):
                        ...
                        if i < len(top_selected):
                            ...
                            raw_ret = m_returns.get(target_sym, -999.0)
                            if raw_ret == -999.0:
                    ...
                    # 3. Calculate subsequent compound holding returns over step interval
                    p_ret_sum = 0.0
                    v_count = 0
                ...
                chart_df = df_bt.set_index("Date")[[f"Top-{top_n_val} Momentum", "Nifty 50 Benchmark"]]
